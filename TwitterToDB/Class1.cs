...
"zippy"
        ...
        static string[] negativeWords = { "2-faced", "2-faces", "abnormal",
"abolish",
"abominable",
"abominably",
"abominate",
...
"zombie" };
        ...
        static Dictionary<string, bool> _stops = new Dictionary<string, bool>
    {
        ...
        private static void SaveTweets(IEnumerable<ITweet> tweets)
        {
            ...
            string[] negativeWords1 = { "2-faced", "2-faces", "abnormal" };

            foreach (var tweet in tweets)
            {
                using (SqlCommand com = new SqlCommand("insert into Tweets(TwitterID,FullText,SentimentType) values(@TwitterID,@FullText,@SentimentType)", connection))
                {
                    com.Parameters.AddWithValue("@TwitterID", tweet.Id);
                    com.Parameters.AddWithValue("@FullText", RemoveStopwords(tweet.FullText));
                    com.Parameters.AddWithValue("@SentimentType", GetSentiment(tweet.FullText));
        ...
        private static string GetSentiment(string input)
        {
            ...
            foreach (string currentWord in words)
            {
                if (negativeWords.Contains(currentWord.ToLower()))
                    negativeCount++;

                if (positiveWords.Contains(currentWord.ToLower()))
                    positiveCount++;
            }
